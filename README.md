...
Форма отправляет JSON на `/api/contact.php`. Скрипт использует почтовый агент Timeweb через PHP `mail()` и отправляет заявки на `hello@ai-tehcon.ru`.

Перед публикацией убедитесь в панели Timeweb, что ящик `hello@ai-tehcon.ru` создан и домен имеет корректные SPF, DKIM и DMARC-записи. Пароль ящика в проекте не используется и не хранится.
